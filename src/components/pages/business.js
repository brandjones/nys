...
import { getBusinessData } from "../../actions/times";
...
class Business extends React.Component {
  ...
  async componentDidMount() {
    const business = await this.props.dispatch(getBusinessData());
  }

  BusinessData = () => {
    return this.props.times.business.map((item, index) => {
      let url;
  ...
  render() {
    return (
      <div className="container">
        <h1 className="forum"> Business </h1>
        <DoubleBorder />
        <Content
