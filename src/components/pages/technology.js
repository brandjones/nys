...
import { getTechnologyData } from "../../actions/times";
...
class Technology extends React.Component {
  ...
  async componentDidMount() {
    const technology = await this.props.dispatch(getTechnologyData());
  }

  TechnologyData = () => {
    return this.props.times.technology.map((item, index) => {
      let url;
  ...
  render() {
    return (
      <div className="container">
        <h1 className="forum"> Technology </h1>
        <DoubleBorder />
        <Content
